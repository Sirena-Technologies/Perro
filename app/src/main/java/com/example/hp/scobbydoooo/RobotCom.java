...
public class RobotCom {
    ...
    public byte[] LUCI_createGeneralPacket(int mbnum, byte[]packet){
            byte[] mbnumbytes = getLHbytes(mbnum);
            int LuciLength = packet.length;
            ...
            System.arraycopy(packet,0,LUCI_packet,10,packet.length);
            return LUCI_packet;
    }

    public void send_general_LUCI_packet(int mbnum, byte[] packet){
        new sendMessageBytes().execute(LUCI_createGeneralPacket(mbnum,packet));
    }

    public void send_general_LUCI_string(int mbnum, String s){
        byte[] data = s.getBytes();
        new sendMessageBytes().execute(LUCI_createGeneralPacket(mbnum,data));
    }
